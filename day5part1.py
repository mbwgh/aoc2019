...
def evaluate(program: List[int], inp: Callable[[], int],
    ...
    def read(mode, pos):
        """Return a value from the current memory, or the value itself."""
        return tape[tape[pos]] if mode == 0 else tape[pos]

    def write(mode, pos, value):
        """
        Either write the value to pos immediately, or write to the address
        written in pos instead.
        """
        if mode == 0:
            tape[tape[pos]] = value
        else:
...
def analyze(number):
    """
    Given a number, return a dict with its opcode, as well as the modes for
    the command's 1st, 2nd and 3rd argument.
    """
    return {
        "opcode": number % 100,
        1: number // 10**2 % 10,
...
def main():
    ...
    program = [int(x) for x in open("day5-input").read().split(",")]
    evaluate(program, inp, out)
    print(output)
