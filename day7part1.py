...
gets sent from amplifier E to the thrusters with the following steps:
    ...
    indicate the amplifier's output signal. Start the software for amplifier B.
    Provide it the phase setting (1) and then whatever output signal was
    produced from amplifier A. It will then produce a new output signal
    destined for amplifier C. Start the software for amplifier C, provide the
    phase setting (2) and the value from amplifier B, then collect its output
    signal. Run amplifier D's software, provide the phase setting (4) and input
    value, and collect its output signal. Run amplifier E's software, provide
    the phase setting (0) and input value, and collect its output signal.
...
def test_combination(phase_settings, program):
    """Evaluate the program chain using the given phase settings."""
    inp_values = None
    inp_pos = 1

    def inp():
        """
        Alternate between returning either the current phase setting,
        or the input.
        """
        nonlocal inp_pos
        inp_pos = (inp_pos + 1) % 2
