...
def parse_input(inp: str) -> List[
    ...
    """
    Given the data inside the input file, split into lines and for each line
    return a pair.
    The first component is a list of requirements.
    Each requirement is a pair (required amount, ingredient name).
    The second component is a process output.
    This is a pair (produced amount, product name).
    :param inp: Data inside the input file.
    :return: A data structure as described above.
    """
    line_pattern = r"([^=]+)=> (\d+ \w+)"
    compound_pattern = r"(\d+) (\w+)"
    ...
    for line in inp.strip().splitlines():
        ...
        inputs = []
        for input_part in line_match.group(1).split(", "):
            compound_match = re.match(compound_pattern, input_part)
            amount = int(compound_match.group(1))
            compound = compound_match.group(2)
            inputs.append((amount, compound))

        output_part = line_match.group(2)
        compound_match = re.match(compound_pattern, output_part)

        amount = int(compound_match.group(1))
        compound = compound_match.group(2)
        result.append((inputs, (amount, compound)))
