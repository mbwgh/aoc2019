...
def rec_fuel(mass: int) -> int:
    """ Calculate the required fuel and take the fuel's weight into account."""
    tot = fuel(mass)
    return 0 if tot <= 0 else tot + rec_fuel(tot)


def total_fuel() -> int:
    """ Calculate the total fuel requirement ."""
    return sum(rec_fuel(int(line)) for line in open("day1-input").readlines())
...
def fuel(mass: int) -> int:
    """ Return the required fuel for this module, depending on its mass."""
    return mass // 3 - 2


def calculate_fuel_input() -> int:
    """ Read the lines of `day1-input` and sum all the fuel requirements."""
    fun = open("day1-input")
    lines = fun.readlines()
